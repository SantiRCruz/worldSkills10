...
import com.example.worldskills10.databinding.ItemPedidoBinding
...
class PedidoAdapter (val pedido : List<DBPedido>): RecyclerView.Adapter<PedidoAdapter.PedidoHolder>() {
    ...
    class PedidoHolder(val view : View,val context: Context): RecyclerView.ViewHolder(view){
        ...
        fun bind(pedido: DBPedido){
            ...
            Glide.with(binding.root).load(pedido.url_imagen).into(binding.img)
            binding.imgRes.setOnClickListener {
                if (pedido.cantidad == 1){
                    ...
                    val res =  dbManager.updateData(pedido.id,pedido.precioUnidad*(pedido.cantidad-1),pedido.cantidad-1)
                    if (res>0){
                        Navigation.findNavController(binding.root).navigate(R.id.navigation_dashboard)
                    }
                }
